...
module.exports = {
	...
	async execute(message, args, d) {
		...
		const item = Object.keys(recipe).find(e => argument.includes(e));
		...
		for (const key in recipe[item]) {
			...
			const orePic = d.emoji[d.ores.tier1.concat(d.ores.tier2, d.ores.tier3).find(v => key.replace('refined ').includes(v))];
