	...
	async execute(message, args, d) {
		...
		if (argument === 'all') {
			function getCost() {
				...
				if (moni === 0 && inv['personal refinery']) { return true; } else { return moni; }
			}
			const cost = getCost();
			...
			const item = argument.replace('all', '');
			...
			const cost = getCostAll();
			if (!inv.ore[`refined ${item}`]) { inv.ore[`refined ${item}`] = inv.ore[item]; } else { inv.ore[`refined ${item}`] += inv.ore[item]; }
			delete inv.ore[item];
			d.addMoni(message.author.id, -cost);
			await d.items.set(message.author.id, inv);
			let oreRefine;
			...
			const regex = /\d+/g;
			const numberOfItems = parseInt(argument.match(regex));
