    ...
    class extends EconomyCommand {
        ...
        async main(msg) {
            ...
            function md5cycle(x, k) {
                ...
                var a = ff(a, b, c, d, k[0], 7, -680876936);
                ...
                var z = kk(d, a, b, c, k[7], 21, -1369616874);
            ...
            function md5rand(num) {
                var rng = new Math.seedrandom(md5(msg.author.id));
                rng()
            ...
            if (result[0] == result[1] && result[1] == result[2]) {
                ...
                this.eco.users.add(msg.author.id, winnings)
            } else {
                // you lose loser
            ...
            // Animate
            async function endMsg(fruits, title, message, edit) {
                const gambleEmbed = new Embed()
                    .setTitle(`${msg.author.username}'s slot machine`)
                    .addField('Fruitz', fruits)
                    .addField(title, message)
                if (edit) {
                    edit.edit(gambleEmbed)
                } else {
                    return await msg.send(gambleEmbed);
                }
            }
            ...
            message = await endMsg(fruitsMsg(0), "Spinning...", "Good luck!", false)
            for (var i = 0; i < r - 1; i++) {
                await timeout(1000)
                await endMsg(fruitsMsg(i), "Spinning...", "Good luck!", message)
            }
            await timeout(1000)
            await endMsg(fruitsMsg(r), "Result", ending, message)
        }
    };
