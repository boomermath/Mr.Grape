module.exports = {
	...
	async execute(message, args, d) {
		...
		async function busted(bet) {
			const bal = await d.users.get(message.author.id);
			const owe = Math.ceil(bal * 0.05);
		...
		const userBal = await d.users.get(message.author.id);
		if (args[0] === 'all') {
		...
		} else if (!isNaN(parseInt(args[0])) && parseInt(args[0]) > 1 && parseInt(args[0]) <= userBal) { 
            decideFate(parseInt(args[0])); } else { message.channel.send('Bruh that\'s not a valid number of stars to bet'); 
        }
	}
};
