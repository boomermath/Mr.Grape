module.exports = {
	...
	async execute(message, args, d) {
		...
		const targetBal = await d.users.get(target.id);
		const robberBal = await d.users.get(message.author.id);
		...
		async function robbery() {
			...
			if (percentage === 1) {
				const earned = Math.floor(+e * targetBal) + 2;
				...
				const loss = Math.floor(+e * robberBal);
