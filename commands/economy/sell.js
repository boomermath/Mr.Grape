module.exports = {
	...
	async execute(message, args, d) {
		...
		const argument = args.join('').toLowerCase().replace(/,/g, '');
		...
		if (Object.keys(d.sellableItems).includes(item) || argument.includes('item' || 'items')) {
			if (argument.includes('item') && argument.includes('all') || argument.includes('items') && argument.includes('all')) {
				...
				const profit = d.sellableItems[item] * inv[item];
				d.addMoni(message.author.id, profit);
				delete inv[item];
				...
				inv[item] -= numItems;
				const profit = d.sellableItems[item] * numItems;
				d.addMoni(message.author.id, profit);
				let receipt;
			...
			if (argument.includes('ores' || 'ore' && 'all') && !oreConcat.some(e => argument.includes(e))) {
				...
				let receipt;
				if (numItems === 1) { receipt = `You sold a ${item} ${d.emoji[oreFromArray]} for ${soldItem[0]} :star:s each!`; } else { receipt = `You sold ${numItems} ${item}s ${d.emoji[oreFromArray]} for ${soldItem[0]} :star:s each!`; }
				const sale = new d.Discord.MessageEmbed()
					.setColor('#dd2de0')
					.setTitle(`${message.author.username}'s sale`)
					.addFields(
