module.exports = {
	...
	async execute(message, args, d) {
		const fruitArray = [
			'apple', 'apricot', 'avocado', 'banana', 'berry', 'blackberry',
			'orange', 'blueberry', 'boysenberry', 'breadfruit', 'cantaloupe',
			'cherry', 'citron', 'citrus', 'coconut', 'crabapple', 'cranberry',
			'currant', 'date', 'durian', 'elderberry', 'fig', 'grape', 'grapefruit',
			'guava', 'honeydew', 'jackfruit', 'kiwi', 'kumquat', 'lemon', 'lime',
			'lingonberry', 'loquat', 'lychee', 'mandarin', 'mango', 'melon',
			'mulberry', 'nectarine', 'papaya', 'peach', 'pear', 'persimmon',
			'pineapple', 'plantain', 'plum', 'pluot', 'pomegranate', 'pomelo', 'prune',
			'quince', 'raisin', 'raspberry', 'strawberry', 'tangelo', 'tangerine', 'watermelon'
		];
		function shuffle(array) {
			...
			if (OGarray.join('') === array.join('')) { return shuffle(array); } else { return array.join(''); }
		}
		const word = fruitArray[Math.floor(Math.random() * fruitArray.length)];
		const scrambledWord = shuffle(word.split(''));
